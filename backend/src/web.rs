...
pub trait AudioImportService: Send + Sync {
    ...
    ) -> Result<audio_input::ImportedAudio, error::WebError>;
}
...
pub trait SpeechTranslationService: Send + Sync {
    async fn translate(
        &self,
        asset_id: audio_input::AudioAssetId,
...
struct AudioInputSpeechAdapter {
    inner: Box<dyn audio_input::AudioInput>,
}

#[async_trait]
impl speech_translation::SpeechTranslationInput for AudioInputSpeechAdapter {
...
impl SpeechTranslationService for SpeechTranslationApplicationService {
    async fn translate(
        ...
        let output = self
            .engine
            .translate_audio(Box::new(AudioInputSpeechAdapter { inner: input }), request)
            .await?;
